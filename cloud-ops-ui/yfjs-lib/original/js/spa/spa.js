...
define('yfjs/spa/util/widget', [
            ...
            if (state) {
                ...
                if (state.path) {
                    if (isScript) {
                        ...
                        // 作为样式文件加载
                        var stylePath = self.normalizeStylesPath(state.url);
    ...
    Widget.prototype.normalizeStylesPath = function(path) {
    ...
    Widget.prototype.Launcher.prototype = {
        ...
        load: function(callback) {
            ...
            $.when(defLoad, defStyles).done(function(argsLoad, argsStyle) {
                ...
                var html = argsLoad[1], bodyHtml = self.getData('__body__');

                if (!_.isUndef(html) && !_.isUndef(bodyHtml)) {
                    html = self.template.render(html, {'__body__': bodyHtml});
                    if (!_.isString(html)) {
                        html = argsLoad[1];
                    }
            ...
            // init styles option
            var stylesOption = widget.normalizeStylesPath(
                instance.callOption('styles')
            );
            if (stylesOption) {
                // load styles
                var styleDefs = [], styleState;
                $.each(stylesOption, function(i, stylePath) {
    ...
    Widget.prototype.Creator.prototype = {
        ...
        load: function(filter, params) {
            ...
            $.each(includes, function(i, stateInclude) {
                ...
                if (params) {
                    for (var k in params) {
                        if (k in stateInclude.param) {
                            stateInclude.param[k] = params[k];
                        } else {
